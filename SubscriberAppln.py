...
class SubscriberAppln():
    ...
    def invoke_operation(self):
        ...
        try:
            ...
            if (self.state == self.State.REGISTER):
                ...
                self.logger.info(
                    "SubscriberAppln::invoke_operation - start checking messages")
                ...
                while True:
                    ...
                    self.logger.info(
                        "SubscriberAppln::invoke_operation - RECEIVING Messages as shown below: {}".format(msg))
                    self.logger.info(
                        "SubscriberAppln::invoke_operation - Current time: {}".format(current_time))
                return None
    ...
    def register_response(self, reg_resp):
        try:
            ...
            if (reg_resp.status == discovery_pb2.STATUS_SUCCESS):
                self.logger.info(
                    "SubscriberAppln::register_respons - registration is a success")
                ...
                self.logger.info(
                    "SubscriberAppln::register_response - registration is a failure with reason {}".format(reg_resp.reason))
    ...
    def receiveSubscribedPublishersResponse(self, lookup_resp):
        try:
            self.logger.info(
                "SubscriberAppln::receiveSubscribedPublishersResponse - start")
...
def parseCmdLineArgs():
    ...
    parser.add_argument("-p", "--port", type=int, default=5574,
                        help="Port number on which our underlying Subscriber ZMQ service runs, default=5576")
    parser.add_argument("-d", "--discovery", default="localhost:5555",
                        help="IP Addr:Port combo for the discovery service, default dht.json")  # changed from localhost:5555
    ...
    # New code for PA2
    parser.add_argument("-j", "--dht_json", default="dht.json",
                        help="JSON file with all DHT nodes, default dht.json")
